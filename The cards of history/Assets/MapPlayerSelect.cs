    ...
    public void SetPlayerSpawns()
    {
        ...
        if (Physics.Raycast(ray, out hit))
        {
            ...
            if (playerSpots[characterIndex - 1] != null)
            {
                ...
                playerSpots[characterIndex - 1].gameObject.GetComponent<Renderer>().material = mapGen.map[(int)tile._ID.y].map[(int)tile._ID.x + ((int)tile._ID.z * mapGen.map[(int)tile._ID.y].gridSizeX)].currentMat;
